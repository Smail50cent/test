...
function encode(toEncode) {
    var finalText = "";
    for (var j = 0; j < ((toEncode.length) / 24); j++) {
        var encodeclone = clone(toEncode);
        var toencoedV24 = encodeclone.substring((j*24), (j * 24)+24);
        finalText += encode24(toencoedV24);
    }
    function encode24(string) {
        var plaintxt = string;
        var oneline;
        ...
        var bf;
        bf = new Blowfish(key);
        for (var f = 0; f < plaintxt.length % 8; f ++)
            plaintxt += "\t";
        oneline = window.btoa(bf.encrypt(plaintxt));
        while (i < oneline.length) {
            formatted_output = formatted_output + oneline.charAt(i);
            i++;
            if (i % 64 == 0) {
    ...
    console.log("encoded ="+toEncode+" to="+finalText);
    return finalText;
}
function decode(encoded) {
    var cipher_b64 = encoded;
    var cipher;
    var plaintext;
    var bf;
    bf = new Blowfish(key);
    cipher_b64 = cipher_b64.replace(/[^A-Za-z0-9\+\/\=]/g, "");
    cipher = window.atob(cipher_b64);
    plaintext = bf.decrypt(cipher);
