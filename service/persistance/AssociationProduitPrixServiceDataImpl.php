...
class AssociationProduitPrixServiceDataImpl implements AssociationProduitPrixServiceData {
    ...
    public function getByProduit($produit) {
        ...
        return $this->parseAssociation($retBdd);
    }

    private function parseAssociation($resultSet) {
        $liste = array();
        $ret;
        while ($ligne = $resultSet->fetch()) {
            ...
            array_push($liste, $associationProduitPrix);
        }
        return $liste;
    }
    ...
    public function getByMenu($menu) {
        $con = new ConnexionBDD();
        $retBdd = $con->executeGeneric("SELECT * FROM association_produit_prix LEFT JOIN prixHt ON prixHt.id = association_produit_prix.prixht_id  LEFT JOIN zone_table ON association_produit_prix.zone_table_id = zone_table.id  WHERE `menu_id` = " . $menu);
        return $this->parseAssociation($retBdd);
    }
