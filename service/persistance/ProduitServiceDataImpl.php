...
");
    ...
    private function testsForListePrix($ligne, Produit $produit) {
        ...
        for ($j = 0; $j < count($produit->getAssociationPrixProduit()); $j++) {
           $asso =  $produit->getAssociationPrixProduit();
            if ($asso[$j]->id == $ligne->association_produit_prix_id) {
                $isHerePrix = true;
                break;
            }
        }if (!$isHerePrix) {
    ...
    private function testsForListeIngredient($ligne, Produit $produit) {
        ...
        for ($j = 0; $j < count($produit->getIngredients()); $j++) {
           $ing =  $produit->getIngredients();
            if (intval($ing[$j]->ingredient) == intval($ligne->association_produit_ingredient_id_ingredient)) {
                $isHereIng = true;
                break;
            }
        }
